...
import { sendBillOnEmail } from "@/services/billing";

const s3 = new S3({
    accessKeyId: process.env.AWS_ACCESS_KEY_ID, // Replace with your AWS Access Key
    secretAccessKey: process.env.AWS_SECREATE_KEY_ID, // Replace with your AWS Secret Key
    region: 'ap-south-1', // Mumbai region (modify if needed)
});
...
const Index: React.FC<BillingPreviewProps> = ({ isOpen, heading, contentData, onClick, detailData, userData }) => {
    ...
    const generatePdf = async (isDownload: boolean) => {
        ...
        if (!isDownload) {
            ...
            const uploadParams = {
                Bucket: 'seeds-feeds',
                Key: `billing/billing-${Date.now()}.pdf`,
                Body: pdfBlob,
    ...
    const sendEmailHandler = React.useCallback(async () => {

        const pdfFilePath = await generatePdf(false);

        const updatedContracts = detailData.contracts.map((contract: any) => ({
            ...contract,
            name: userData?.name,
        }));
        ...
        >
                            ...
                            <tbody>
                                {contentData?.contracts.map((contract: any, index: number) => (
                                    <tr key={index}>
                                        ...
                                        <td>{contract.partyType === 'Seller' ? contract.contractDetails.buyer_id.name : contract.contractDetails.seller_id.name}</td>
            ...
            <DialogActions>
                <div className="preview-pop-ok-btn billing-preview-pop">
                    ...
                    <Button variant="outlined" onClick={() => generatePdf(true)}>Download Pdf</Button>
