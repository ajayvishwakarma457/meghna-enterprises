...
const Index: React.FC<compProps> = ({ detail, updatedDetail }) => {

  const router = useRouter();
  const [detailData, setDetailData] = useState<DetailData>(updatedDetail.data as DetailData);
  ...
  return (
    <>
                ...
                <form  className="form billing-form">
                  ...
                  {detailData.contracts && detailData.contracts.length > 0 && detailData.contracts.map((contract: any, index: number) => (
                    ...
                    <TextField
                        ...
                        fullWidth
                        margin="normal"
                        value={contract.partyType === 'Seller' ? contract.contractDetails.buyer_id.name :contract.contractDetails.seller_id.name}                    
                        disabled={true}    
                    />
      ...
      <BillingPreviewDialogue isOpen={isPreviewDialogOpen} heading="Contract Preview" contentData={previewContent} onClick={previewClickHandler} userData={userData} detailData={detailData} />
...
export const getServerSideProps: GetServerSideProps = async (context: GetServerSidePropsContext) => {
  ...
  const detail:any = await getBilling(id as string); 
  const contractIds = (detail as any).data.contracts.map((contract:any) => contract.contractId);
  
  const getContractsDetails = async (contractIds: string[]) => {
    try {
      const contractDetails = await Promise.all(contractIds.map(async (id) => await getContract(id)));      
  ...
  const contracts:any = await getContractsDetails(contractIds);
  const updatedDetail = {
    ...detail,
    data: {
      ...detail.data,
      contracts: (detail as any).data.contracts.map((contract:any) => {
        const matchingContract = contracts.find((c:any) => c.data._id === contract.contractId);
        return matchingContract ? { ...contract, contractDetails: matchingContract.data } : contract;
      }),
    },
  };
